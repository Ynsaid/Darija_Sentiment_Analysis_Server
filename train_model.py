...
- Conv1D: extracts local patterns (n-grams) in text.
- GlobalMaxPooling1D: keeps most informative features.
- Dense + Dropout: for classification with reduced overfitting.
"""

# model = Sequential([
#     # Larger embedding to capture code-switching
#     #update 1 : from 300 to 128
#     Embedding(input_dim=MAX_VOCAB, output_dim=128, input_length=MAX_LEN),
#     # from 0.2 to 0.3
#     SpatialDropout1D(0.3),
    
#     # Multi-scale convolutions (capture different n-grams)
#     Conv1D(128, 3, activation='relu', padding='same', kernel_regularizer=l2(1e-4)),
#     BatchNormalization(),
#     Dropout(0.3),
...
# ])
#pre training is bad 
#update remove l2 and improve epoch 
#update 3 50 epoch with 35 stop early
model = Sequential([
    Embedding(input_dim=MAX_VOCAB, output_dim=128, input_length=MAX_LEN),
    SpatialDropout1D(0.3),
...
# to verify if model is update loss or no 
# after try 3  the model stop at 3 
# after try 10 the model complete until 12 
